  ...
  return (
    <main className="mx-auto flex max-w-4xl flex-col gap-6 px-6 py-12">
      <header className="space-y-2 border-b border-white/5 pb-4">
        ...
        <p className="text-sm text-slate-300">{translate(locale, "ui.startQuestionnaire")}</p>
      </header>
      <section className="space-y-4">
        {section.subcategories.map((subcategory) => (
          <article key={subcategory.id} className="rounded-xl border border-white/10 bg-slate-900/70 p-5">
            <h2 className="text-lg font-semibold">{translate(locale, subcategory.titleKey)}</h2>
            <p className="text-sm text-slate-400">{translate(locale, subcategory.descriptionKey)}</p>
